...
class Conta {
    ...
    transferir(contaDestino, valor) { this.sacar(valor); contaDestino.depositar(valor); }
...
class Banco {
    ...
    inserirCliente(cliente) {
        ...
        if (existe) {
            console.log("Erro: cliente já cadastrado!");
            return;
        }
    ...
    inserirConta(conta) {
        ...
        if (existe) {
            console.log("Erro: conta já existente!");
            return;
        }
    ...
    transferir(numeroOrigem, numeroDestino, valor) {
        ...
        if (origem && destino) {
            ...
            console.log("Conta origem ou destino não encontrada.");
    }
    ...
    quantidadeContas() { return this.contas.length; }
    totalDinheiro() { return this.contas.reduce((t, c) => t + c.saldo, 0); }
    mediaSaldos() { return this.totalDinheiro() / this.quantidadeContas(); }
    mudarTitularidade(numero, novoCpf) {
        const conta = this.consultarConta(numero);
        const novoCliente = this.clientes.find(c => c.cpf === novoCpf);
        if (conta && novoCliente) {
            conta.cliente = novoCliente;
            console.log("Titularidade alterada.");
        }
        else
    ...
    carregarDados() {
        ...
        const a = new Conta(1, "100", new Date(), 1000);
        a.cliente = c1;
        const b = new Conta(2, "200", new Date(), 2000);
        b.cliente = c2;
        const c = new Conta(3, "300", new Date(), 3000);
        c.cliente = c3;
        const d = new Conta(4, "400", new Date(), 4000);
        d.cliente = c4;
        const e = new Conta(5, "500", new Date(), 5000);
        e.cliente = c5;
